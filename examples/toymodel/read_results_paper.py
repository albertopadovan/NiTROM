...
for k in range(len(betas_train)):
    ...
    y_pod = fom.compute_output(sol_pod)
    error_pod_train += (
        np.linalg.norm(y_pod - y_fom, axis=0) ** 2 / weight / len(betas_train)
    )
    ...
    y_oi = fom.compute_output(sol_oi)
    error_oi_train += (
        np.linalg.norm(y_oi - y_fom, axis=0) ** 2 / weight / len(betas_train)
    )
    ...
    y_oi_gs = fom.compute_output(sol_oi_gs)
    error_oi_gs_train += (
        np.linalg.norm(y_oi_gs - y_fom, axis=0) ** 2 / weight / len(betas_train)
    )
    ...
    y_nit = fom.compute_output(sol_nit)
    error_nit_train += (
        np.linalg.norm(y_nit - y_fom, axis=0) ** 2 / weight / len(betas_train)
    )
    ...
    y_nit_gs = fom.compute_output(sol_nit_gs)
    error_nit_gs_train += (
        np.linalg.norm(y_nit_gs - y_fom, axis=0) ** 2 / weight / len(betas_train)
    )
