...
printr(
    f"Running sweep over {len(regs)} regularization parameters from {regs[0]} to {regs[-1]}..."
)
printr("-" * 75)
printr(
    f"{'Regularization':<20} | {'OpInf NiTROM Cost':<22} | {'GAS-OpInf NiTROM Cost':<22}"
)
printr("-" * 75)
...
printr("-" * 75)
printr(
    f"Optimal standard OpInf regularization: {best_opinf_reg:.6e} with NiTROM Cost: {best_opinf_cost:.6e}"
)
printr(
    f"Optimal GAS-OpInf regularization:      {best_gas_reg:.6e} with NiTROM Cost: {best_gas_cost:.6e}"
)
printr("-" * 75)
...
if rank == 0:
    ...
    if best_opinf_tensors is not None:
        save_checkpoint(
            best_opinf_tensors, "opinf", os.path.join(models_dir, "opinf_model.pkl")
        )
