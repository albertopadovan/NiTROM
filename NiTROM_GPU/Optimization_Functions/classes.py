...
class pool:
        ...
        **kwargs,
        ...
        if device is None:
            ...
            if rank is None or world_size is None:
                raise ValueError(
                    "If you provide a device, you also have to provide rank and world_size"
                )
            self.rank = rank
    ...
    def load_weights(self, kwargs):
        ...
        if fname_weights != None:
            ...
            weights = [
                np.load(self.fnames_weights[k]) for k in range(len(self.fnames_weights))
            ]
            for k in range(self.my_n_traj):
    ...
    def load_steady_forcing(self, kwargs):
        ...
        if fname_forcing != None:
            ...
            for k in range(self.my_n_traj):
                F[:, k] = np.load(self.fnames_forcing[k])
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.l2_pen != None and 1 not in self.poly_comp:
            raise ValueError(
                "The penalty is currently implemented for the linear term \
    ...
    def _global_to_local_indices(self, global_indices, pool):
        ...
        if not isinstance(global_indices, torch.Tensor):
            global_indices = torch.tensor(
                global_indices, device=pool.device, dtype=torch.long
            )
    ...
    def evaluate_rom_rhs(self, t, z, u, *operators, **kwargs):
        ...
        if torch.linalg.vector_norm(z) >= 1e6:
            ...
            f = (
                f(t)
                if f != None
                else torch.zeros(len(z), device=z.device, dtype=z.dtype)
            )
