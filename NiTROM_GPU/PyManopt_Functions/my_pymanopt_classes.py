...
class myAdaptiveLineSearcher:
    ...
    def search(self, objective, manifold, x, d, f0, df0):
        ...
        try:
            ...
            if self._rank == 0:
                print("Singular matrix encountered, reducing step size.")
        ...
        if alpha <= 1e-12:
            ...
            while newf > 1.01 * f0 and cost_evaluations <= self._max_iterations:
                # Reduce the step size.
                alpha *= self._contraction_factor
...
class myGPUAdaptiveLineSearcher:
    ...
    def search(self, objective, manifold, x, d, f0, df0):
        ...
        try:
            ...
            if self._rank == 0:
                print(f"Error during retraction: {e}. Reducing step size.")
            alpha *= self._contraction_factor
            newx_torch = self._manifold_retraction(manifold, x_torch, d_torch, alpha)
            newx = self._to_numpy_tuple(newx_torch)
        ...
        while (
            ...
            # Look closer down the line.
            try:
                newx_torch = self._manifold_retraction(
                    manifold, x_torch, d_torch, alpha
                )
                newx = self._to_numpy_tuple(newx_torch)
            except Exception as e:
                if self._rank == 0:
    ...
    def _gpu_norm(self, manifold, x, d):
        ...
        if manifold.__class__.__name__ == "Stiefel":
            ...
            return torch.tensor(
                manifold.norm(x.cpu().numpy(), d.cpu().numpy()), device=self._device
            )
    ...
    def _manifold_retraction(self, manifold, x, d, alpha):
        ...
        if hasattr(manifold, "manifolds"):
            return tuple(
                self._gpu_retraction(manifold.manifolds[i], x[i], d[i], alpha)
