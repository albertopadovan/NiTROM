...
def create_objective_and_gradient(manifold, opt_obj, pool, fom):
    ...
    def cost(*params):
        ...
        Phi = Phi.to(pool.device)
        Psi = Psi.to(pool.device)
        ...
        for k in range(pool.my_n_traj):
            ...
            J += (1.0 / opt_obj.weights[k]) * torch.trace(e.T @ e)
    ...
    def euclidean_gradient(*params):
        ...
        Phi = torch.from_numpy(Phi).to(pool.device)
        Psi = torch.from_numpy(Psi).to(pool.device)
        ...
        t_unit = torch.linspace(
            0.0, 1.0, steps=opt_obj.nsave_rom, device=pool.device, dtype=torch.float64
        )
        ...
        tlg = torch.from_numpy(tlg).to(pool.device)
        wlg = torch.from_numpy(wlg).to(pool.device)
            ...
            for j in range(opt_obj.n_snapshots - 1):
                ...
                for i in range(opt_obj.leggauss_deg):
                    ...
                    for count, p in enumerate(opt_obj.poly_comp):
                        ...
                        grad_tensors[count] -= (
                            a * wlg[i] * torch.einsum(equation, *operands)
                        )
...
def check_gradient_using_finite_difference(
    M, Phi, Psi, A2, A3, A4, opt_obj, mpi_pool, fom, eps
):
    ...
    dfd = (0.5 / eps) * (
        cost(Phi + eps * delta, Psi, A2, A3, A4)
        - cost(Phi - eps * delta, Psi, A2, A3, A4)
    )
    ...
    if mpi_pool.rank == 0:
        print("------ Error for Phi ------------")
        print(
            "dfd = %1.5e,\t dfgrad = %1.5e,\t error = %1.5e,\t percent error = %1.5e"
            % (dfd, dgrad, error, percent_error)
        )
        print("---------------------------------")
    ...
    dfd = (0.5 / eps) * (
        cost(Phi, Psi + eps * delta, A2, A3, A4)
        - cost(Phi, Psi - eps * delta, A2, A3, A4)
    )
    ...
    dfd = (0.5 / eps) * (
        cost(Phi, Psi, A2 + eps * delta, A3, A4)
        - cost(Phi, Psi, A2 - eps * delta, A3, A4)
    )
    ...
    dfd = (0.5 / eps) * (
        cost(Phi, Psi, A2, A3 + eps * delta, A4)
        - cost(Phi, Psi, A2, A3 - eps * delta, A4)
    )
    ...
    dfd = (0.5 / eps) * (
        cost(Phi, Psi, A2, A3, A4 + eps * delta)
        - cost(Phi, Psi, A2, A3, A4 - eps * delta)
    )
